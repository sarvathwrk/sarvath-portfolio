...
/**
 * A single value for a CSS custom property / theme token
 */
export type ThemeTokenValue = string | number;

/**
 * A map of theme token names to their values
 */
export type ThemeTokens = Record<string, ThemeTokenValue>;

/**
 * Tokens that change based on the active color theme
 */
export type Theme = {
  themeId: ThemeId;
  ...
  // Themes are a bag of CSS custom property tokens; allow additional string
  // tokens so consumers can read theme-specific values not listed above.
  [token: string]: string;
};
...
export const theme: Record<ThemeId, Theme> = { dark, light };
