// nodemailer ships no type declarations and no @types package is installed,
// so `require` intentionally yields `any` here (avoids a "no declaration file"
// error that an ESM `import` would raise).
const nodemailer = require('nodemailer');

/**
 * Payload submitted from the contact form.
 */
...
export async function sendMail(data: ContactFormPayload): Promise<boolean> {
  try {
